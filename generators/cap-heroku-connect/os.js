...
jwtCommandWindows = `curl -X PUT -d " https://www.googleapis.com/service_accounts/v1/metadata/x509/securetoken@system.gserviceaccount.com "| jq '[ to_entries | .[] | {alg: "RS256", kty: "RSA", use: "sig", kid: .key, x5c: [(.value | sub(".*"; "") | sub("\n"; ""; "g") | sub("-.*"; "")) ] } ] | {"keys": .}'`
...
if(opsys === 'darwin' || opsys === 'linux'){
  ...
  let jwt = await exec(jwtCommandWindows);
    let jsonJWT = JSON.stringify(JSON.parse(jwt.stdout));
    load.stop();
    load.succeed('The JSON was successful generated.');
    return jsonJWT;
...
const installJq = async (command) => {
  try {
    ...
    if (installationStatus.stdout.includes('Already downloaded') || installationStatus.stdout.includes('Downloaded')) {
      load.stop();
